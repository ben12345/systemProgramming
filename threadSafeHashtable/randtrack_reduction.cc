...
void *thread(void *arg) {
  ...
  int rnum;
  int tid = (*((int *)arg));
  hash<sample,unsigned> *cur = &h[tid];
...
main (int argc, char* argv[]){
  ...
  h = new hash<sample,unsigned>[num_threads];
  ...
  for (i = 0; i < num_threads; i++) {
    cur = &h[i];
      ...
      if ((s = cur->lookup(key))) { // key has been counted in i
        if (!(m = merge.lookup(key))) { // key has NOT been counted in merge
          m = new sample(key);
				  merge.insert(m);
        }
        m->count += s->count;
